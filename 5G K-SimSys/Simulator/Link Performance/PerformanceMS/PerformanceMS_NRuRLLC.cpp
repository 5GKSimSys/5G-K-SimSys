...
void PerformanceMS::Measure()
{
	...
	double temp = MS[id]->scheduling->downlinkESINR * (1 - BS[MS[id]->network->attachedBS]->scheduling->numScheduledUMS / Sim.scheduling->numCB);
	if (arma::randu() > FER(temp, MS[id]->scheduling->MCS))
	{
		instantThroughput = MS[id]->scheduling->downlinkspectralEfficiency * Sim.channel->NRuRLLC.bandwidth / 10 * 50 * (12 * 11 - 12 - 16) * 1000;
