...
#include "PerformanceMS_NReMBB.h"
...
void PerformanceMS::Measure(int link)
{
	MS[id]->scheduling->ReceivedSINR(id, Sim.TTI % Sim.numTTIperFrame, 0);
	double temp = MS[id]->scheduling->downlinkESINR;
	if (arma::randu() > FER(temp, MS[id]->scheduling->MCS))
	{
		if (link == 0)
		{
			instantThroughput = MS[id]->scheduling->downlinkspectralEfficiency * Sim.channel->NReMBB.bandwidth / 10 * 50 * (12 * 11 - 12 - 16) * 1000;
			MS[id]->scheduling->downlinkHARQeSINR = 0;
		}
		else
		{
			instantThroughput = MS[id]->scheduling->uplinkspectralEfficiency * Sim.channel->NReMBB.bandwidth / 10 * 50 * (12 * 11 - 12 - 16) * 1000;
			MS[id]->scheduling->uplinkHARQeSINR = 0;
		}
	...
	else
	{
		...
		{
			instantThroughput = 0;
			MS[id]->scheduling->downlinkHARQeSINR = MS[id]->scheduling->downlinkHARQeSINR + temp;
		}
		else
		{
			instantThroughput = 0;
			MS[id]->scheduling->uplinkHARQeSINR = MS[id]->scheduling->downlinkHARQeSINR + temp;
		}
	}
	if (link == 0)
		downlinkThroghput = downlinkThroghput * (Sim.TTI) / (Sim.TTI + 1) + instantThroughput / (Sim.TTI + 1);
	else
		uplinkThroghput = uplinkThroghput * (Sim.TTI) / (Sim.TTI + 1) + instantThroughput / (Sim.TTI + 1);
}
