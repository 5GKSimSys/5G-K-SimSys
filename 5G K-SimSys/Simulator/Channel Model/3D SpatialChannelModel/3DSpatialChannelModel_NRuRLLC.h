...
3D Spatial Channel Model header (NR uRLLC)


PROJECT NAME : System Level Simulator High Level Design
FILE NAME    : 3DSpatialChannelModel_NRuRLLC.h
DATE         : 2017.7.16
VERSION      : 0.11
...
// Channel Level 3 functions - actual channel implementations (TR 38.901)
	void Pathloss(int src, int dst, int type);
	void Delay(int src, int type);
	void ClusterPower(int src, int dst, int type);
	void ArrivalAndDepartureAngle(int src, int bs, int site, int sector, int type);
	void ArrivalAndDepartureAntennaGain(int src, int bs, int site, int sector, int type);
	double AntennaGain(arma::mat angleLCS);
	arma::mat ReceiverAntennaGain(int src, int zeta, arma::mat angleGCS, int type);
	...
	arma::cx_mat exp_F_urd(int src, arma::mat A, int numRxAntenna, int type);
