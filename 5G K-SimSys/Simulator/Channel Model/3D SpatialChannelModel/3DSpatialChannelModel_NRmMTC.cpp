...
Channel Model for Mobile Station (NR mMTC)


PROJECT NAME : System Level Simulator
FILE NAME    : SpatialChannel_NRmMTC.cpp
DATE         : 2017.01.02
VERSION      : 0.0
...
#include "3DSpatialChannelModel_NRmMTC.h"
...
//	}
//
//	double couplingLoss = pow(10.0, -MS[src]->channel->pathloss / 10.0) * pow(10.0, -MS[src]->channel->largeScaleParameter(0, 0) / 10.0)* pow(10, (Sim.channel->NRmMTC.txPower / 10.0));
...
void SpatialChannel::Pathloss(int src, int dst, int type) {
	...
	double breakPointDistance2D = 4.0 * (dstZ - 1.0) * (srcZ - 1.0) * (Sim.channel->NRmMTC.carrierFrequency*1e9) / 3e8;
	...
	breakPointDistance2D = 4.0 * (dstZ - 1.0) * (srcZ - 1.0) * (Sim.channel->NRmMTC.carrierFrequency*1e9) / 3e8;

	if (distance2D >= 10 && distance2D <= breakPointDistance2D) {
		pathlossLOS = 28.0 + 22.0 * log10(distance3D) + 20.0 * log10(Sim.channel->NRmMTC.carrierFrequency);
	}
	else if (breakPointDistance2D < distance2D && distance2D < 5000.0) {
		pathlossLOS = 40.0 * log10(distance3D) + 28.0 + 20.0 * log10(Sim.channel->NRmMTC.carrierFrequency) - 9.0 * log10(pow(breakPointDistance2D, 2.0) + pow((srcZ - dstZ), 2.0));
	}
	pathlossNLOS = 13.54 + 39.08 * log10(distance3D) + 20.0 * log10(Sim.channel->NRmMTC.carrierFrequency) - 0.6 * (dstZ - 1.5);
