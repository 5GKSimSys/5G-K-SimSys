...
Channel Model for Mobile Station (NR eMBB)
...
void ChannelMS::RSRP(int src, int dst, int site, int sector) {
	...
	case SLS::IndoorOffice:
		...
		if ((MS[src]->channel->channelCondition == LOS)) { // Outdoor LOS
			muXPR = spatialChannel.IndoorOfficeLOS.muXPR;
			sigmaXPR = spatialChannel.IndoorOfficeLOS.sigmaXPR;
		}
		else if ((MS[src]->channel->channelCondition == NLOS)) {
			muXPR = spatialChannel.IndoorOfficeNLOS.muXPR;
			sigmaXPR = spatialChannel.IndoorOfficeNLOS.sigmaXPR;
		}
	...
	case SLS::RuralMacroCell:
		...
		if ((MS[src]->network->location == Outdoor) && (MS[src]->channel->channelCondition == LOS)) { // Outdoor LOS
			muXPR = spatialChannel.RuralMacroCellLOS.muXPR;
			sigmaXPR = spatialChannel.RuralMacroCellLOS.sigmaXPR;
		}
		else if ((MS[src]->network->location == Outdoor) && (MS[src]->channel->channelCondition == NLOS)) {
			muXPR = spatialChannel.RuralMacroCellNLOS.muXPR;
			sigmaXPR = spatialChannel.RuralMacroCellNLOS.sigmaXPR;
		}
		else if ((MS[src]->network->location == Indoor) && (MS[src]->channel->channelCondition == LOS)) {
			muXPR = spatialChannel.RuralMacroCellOtoI.muXPR;
			sigmaXPR = spatialChannel.RuralMacroCellOtoI.sigmaXPR;
		}
		else {
			muXPR = spatialChannel.RuralMacroCellOtoI.muXPR;
			sigmaXPR = spatialChannel.RuralMacroCellOtoI.sigmaXPR;
	...
	arma::vec RSRP_cal;
	RSRP_cal.zeros(Sim.network->numBS);
	arma::mat RSRP;
...
void ChannelMS::ApplyPathLossAndShadowing(int src) {
	arma::cx_mat x(Sim.network->numBS, 1);
	arma::cx_mat y(1, 1);
...
void ChannelMS::DftParameterSetting(int src)
{
	...
	for (int si = 0; si < Sim.network->numBS; si++)
	{
		...
		for (int c = 0; c < 50/10*Sim.channel->NReMBB.bandwidth; c++)
		{
