...
Channel Model (NR uRLLC)


PROJECT NAME : System Level Simulator
FILE NAME    : Channel_NRuRLLC.cpp
DATE         : 2017.1.27
VERSION      : 0.0
...
void Channel::Initialize(string fileName)
{
	...
	if (inFile.is_open())
	{
		while (!inFile.eof()) {
			...
			if (result[0] == "carrierFrequency") NRuRLLC.carrierFrequency = stod(result[1]);
			else if (result[0] == "bandwidth") NRuRLLC.bandwidth = stod(result[1]);
			else if (result[0] == "txPower") NRuRLLC.txPower = stod(result[1]);
			else if (result[0] == "rxPower") NRuRLLC.rxPower = stod(result[1]);
			else if (result[0] == "noiseFigure") NRuRLLC.noiseFigure = stod(result[1]);
			...
			if (Sim.channel->ChannelModel == 1) {
				if (result[0] == "carrierFrequencyOfSmallCell") NRuRLLC.carrierFrequencyOfSmallCell = stod(result[1]);
				else if (result[0] == "bandwidthOfSmallCell") NRuRLLC.bandwidthOfSmallCell = stod(result[1]);
				else if (result[0] == "txPowerOfSmallCell") NRuRLLC.txPowerOfSmallCell = stod(result[1]);
				else if (result[0] == "rxPowerOfSmallCell") NRuRLLC.rxPowerOfSmallCell = stod(result[1]);
				else if (result[0] == "noiseFigureOfSmallCell") NRuRLLC.noiseFigureOfSmallCell = stod(result[1]);
		...
		WaveLengthOfElectomageneticWave = SpeedOfElectomageneticWave / (NRuRLLC.carrierFrequency*1e9);	
...
void Channel::LongTermChannel()
{
	...
	for (int msID=0; msID<Sim.network->numMS; msID++)
	{
		...
		cout << "[Channel " << longTermChannelProcess << "%]: Long Term Channel of eMBB MS ID " << msID << " Calculated" << endl;	
	}	

	for (int umsID = 0; umsID<Sim.network->numUMS; umsID++)
	{
		TemporaryGlobalVariableInitialize(umsID, 1);
		UMS[umsID]->channel->LongTermChannel(umsID);

		setcolor(11, 0);
		double longTermChannelProcess;
		longTermChannelProcess = 95 * umsID / (Sim.network->numUMS);
		cout << "[Channel " << longTermChannelProcess << "%]: Long Term Channel of uRLLC MS ID " << umsID << " Calculated" << endl;
	}
}
...
void Channel::TemporaryGlobalVariableInitialize(int msID, int type) {
	...
	{
		UMS[msID]->channel->RSRPout.zeros(57, 1);
		UMS[msID]->channel->VelocityLOS.zeros(57, 1);
		UMS[msID]->channel->Alpha_zero_temp = arma::field<arma::cx_mat>(Sim.network->numBS, 1);
		UMS[msID]->channel->Alpha_zero_temp_x = arma::field<arma::cx_mat>(Sim.network->numBS, 1);
		UMS[msID]->channel->Alpha_zero_temp1 = arma::field<arma::cx_mat>(Sim.network->numBS, 1);
		UMS[msID]->channel->Alpha_zero_temp_x1 = arma::field<arma::cx_mat>(Sim.network->numBS, 1);
		UMS[msID]->channel->NumRealCluseter.zeros(Sim.network->numBS, 1);
		arma::field<arma::cx_mat> A(Sim.network->numBS, 1);
		UMS[msID]->channel->F_urd_LOS = A;
		UMS[msID]->channel->F_prd_LOS = A;
		arma::field<arma::cx_mat> B(MAX_CLUSTER, 20);
		UMS[msID]->channel->F_urd = B;
		UMS[msID]->channel->F_prd = B;
		arma::field<arma::cx_mat> C(Sim.network->numBS, MAX_CLUSTER, 20);
		UMS[msID]->channel->F_urd_BS = C;
		UMS[msID]->channel->F_prd_BS = C;
		arma::field<arma::cx_mat> D(Sim.network->numBS, Sim.channel->NumberOfChannelProcess);
		UMS[msID]->channel->Alpha_nmup_temp = D;
		UMS[msID]->channel->Alpha_nmup_temp_x = D;
		UMS[msID]->channel->Alpha_nmup_temp1 = D;
		UMS[msID]->channel->Alpha_nmup_temp_x1 = D;
		UMS[msID]->channel->VelocityBS = arma::field<arma::mat>(Sim.network->numBS, MAX_CLUSTER + 4, 20);
		UMS[msID]->channel->tau = arma::field<arma::mat>(57);
		UMS[msID]->channel->tauLOS = arma::field<arma::mat>(57);
		UMS[msID]->channel->DftParameter = arma::field<arma::cx_mat>(Sim.network->numBS, 50);
		UMS[msID]->channel->FrequencyChannel = arma::field<arma::cx_mat>(Sim.network->numUMS, 2, 50);
		UMS[msID]->channel->CouplingLoss.zeros(57);
		arma::field<arma::mat> E(Sim.network->numBS, 1);
		UMS[msID]->channel->DemoAOA = E;
		UMS[msID]->channel->DemoAOD = E;
		UMS[msID]->channel->DemoClusterPower = E;
	}
}
