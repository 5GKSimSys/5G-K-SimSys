...
Network Configuration for Mobile Station (NR uRLLC)


PROJECT NAME : System Level Simulator
FILE NAME    : NetworkURLLCMS_NRuRLLC.cpp
DATE         : 2016.10.11
VERSION      : 0.0
...
#include "NetworkURLLCMS_NRuRLLC.h"
...
void NetworkURLLCMS::PlaceRandomHexagonal()
{
...
void NetworkURLLCMS::WrapAround()
{
	//int id = ms; // MS ID
	...
	for (int i = 0; i < Sim.network->numSite; i++) // Long term channel calculation
	{
		...
		{
			vector = abs(UMS[id]->network->pos - (Sim.network->posWraparound[j] + BS[Sim.network->numSector * i]->network->pos));
			temp(j) = vector;
		}
...
arma::mat NetworkURLLCMS::LargeScaleParameter(int channelModel, int channelCondition, int msID, int bsID) {
	...
	if ((UMS[msID]->network->location == Outdoor) && (UMS[msID]->channel->channelCondition == LOS)) {
	...
	else if ((UMS[msID]->network->location == Outdoor) && (UMS[msID]->channel->channelCondition == NLOS)) {
	...
	else if ((UMS[msID]->network->location == Indoor)) {
...
void NetworkURLLCMS::Associate(int msID, arma::vec RSRP)
{
	...
	UMS[msID]->channel->associatedBsIndex = indices(0);
	BS[indices(0)]->network->attachedUMS[BS[indices(0)]->channel->NumAssociatedUMS] = msID;
	BS[indices(0)]->channel->NumAssociatedUMS = BS[indices(0)]->channel->NumAssociatedUMS + 1;
	UMS[msID]->channel->BSindex = interferenceIndex;
}

void NetworkURLLCMS::ConcludeIteration()
{

}

void NetworkURLLCMS::Conclude() {
